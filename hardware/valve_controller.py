...
通过 GPIO 控制高电平触发的继电器模块，进而驱动 12V 电磁阀控制水开关。
设计原则：
  * 软硬限位（最大单次时长、冷却、全局互锁）保证不会因为软件 bug 把电磁阀烧掉。
...
class _RpiGpioBackend(_GpioBackend):
    ...
    def setup(self, pin: int) -> None:
        self._GPIO.setup(pin, self._GPIO.OUT, initial=self._GPIO.LOW)  # 初始 LOW = 继电器断开

    def write(self, pin: int, value: int) -> None:
        # 1 = 继电器吸合（开阀），0 = 继电器断开（关阀）
...
class _GpiodBackend(_GpioBackend):
    ...
    def setup(self, pin: int) -> None:
        ...
        req = gpiod.request_lines(
            self._chip_path, config={pin: settings}, consumer="valve"
        )
    ...
    def write(self, pin: int, value: int) -> None:
        ...
        # ACTIVE → HIGH → 继电器吸合（开阀）, INACTIVE → LOW → 继电器断开（关阀）
...
class ValveController:
        ...
        *,
        ...
        for c in channels:
            ch = Channel(
                ...
                pin=int(c["pin"]),
                bucket=c.get("bucket", ""),
                default_duration_sec=int(c["default_duration_sec"]),
    ...
    def _clamp_duration(self, duration_sec: int) -> float:
        return min(float(duration_sec), self.max_single_run_sec)

    def water(self, channel_id: int, duration_sec: Optional[int] = None) -> Dict:
        if channel_id not in self.channels:
            raise ValueError(f"未知通道 id={channel_id}")
        ch = self.channels[channel_id]
        duration = int(duration_sec) if duration_sec else ch.default_duration_sec
        ...
        seconds = self._clamp_duration(duration)

        glock = self._global_lock if self._global_lock else None
        if glock and not glock.acquire(timeout=1):
            raise RuntimeError("已有电磁阀在运行，本次请求被拒绝")

        try:
            with ch.lock:
                ch.is_running = True
                logger.info("通道 %s 开始浇水: 目标 %ss → 运行 %.1fs", ch.name, duration, seconds)
                ...
                ch.last_duration_sec = int(seconds)
                ch.is_running = False
        finally:
            if glock:
                glock.release()

        logger.info("通道 %s 浇水完成: 实际 %.1fs", ch.name, seconds)
        return {
            "channel_id": ch.id,
            "name": ch.name,
    ...
    def status(self) -> List[Dict]:
        ...
        for ch in self.channels.values():
            out.append({
                ...
                "last_duration_sec": ch.last_duration_sec,
